#!/bin/bash
#SBATCH --job-name=train-languided    # create a short name for your job
...
#SBATCH --output=logs_mosmed/%x_%j.out  # output
#SBATCH --error=logs_mosmed/%x_%j.err   # error

echo "My SLURM_ARRAY_JOB_ID is $SLURM_ARRAY_JOB_ID."
echo "My SLURM_ARRAY_TASK_ID is $SLURM_ARRAY_TASK_ID"
echo "Executing on the machine:" $(hostname)
echo "Test ClipSeg encoder + ClipSeg decoder, save_model_mosmed/v11"
...
python train.py --config config/training_mosmed.yaml
# python evaluate.py --config config/training_mosmed.yaml